...
function update_table(new_table, old_table, name, deep, replace)
    ...
    if check_updated(new_table, old_table, name, deep) then 
        M.log_debug("new %s old %s name %s is updated", _tostring(new_table), _tostring(old_table), name)
        return 
    end
    ...
    for key, value in pairs(new_table) do
        ...
        elseif type_value == "table" then
            update_table(value, old_value, key, deep, replace)
        end
    end  -- for
...
function M.hotfix_module(module_name, replace)
    assert("string" == type(module_name))
    M.log_debug("Hot fix module: %s replace %s", module_name, replace)
    add_self_to_protect()
